...
function Agent(config){
  ...
  this.warn_command        = config.warn_command;
...
Agent.prototype.switch_master_handler = function(){
  ...
  return function(data) {
    ...
    //warn command
    if(self.warn_command != '') {
      var warn = "switch-master "+data.details["master-name"]+" "+data.details["old-ip"]+":"+data.details["old-port"]+" => "+data.details["new-ip"]+":"+data.details["new-port"];
      var content = self.warn_command + "\"" + warn + "\"";
      var child = exec(
        content,
        function(error, stdout, stderr) {
          self.log("warn command with output: ");
          self.log(content);
          if (error !== null) {
            self.log("warn command with error: " + error);
          }
        }
      );
    }
